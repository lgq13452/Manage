  ...
  <div class="register">
    ...
    <div class="register-box">
      ...
      <div class="form-box">
        ...
        <div class="form-group clearfix">
          <label for="verification-code" class="fl form-title code-text">验证码</label>
          <input
            type="text"
            class="form-control fl form-input verification-code"
          ...
          <button
            type="button"
            class="btn btn-secondary get-code fr"
            :disabled="isSend"
            @click="getCode"
          >{{text}}</button>
        </div>
        ...
        <div class="form-group clearfix form-btn">
          <button type="button" class="btn btn-success login fr" @click="register">注册</button>
        </div>
        <div class="switch">
          <router-link :to="{name: 'Login'}"> 已有账号立即登录</router-link>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getCode() {
      ...
      let res = verification.verificat(data);
      if (res.pass === false) {
        this.$showTotal({
          msg: res.msg,
          duration: 3000
